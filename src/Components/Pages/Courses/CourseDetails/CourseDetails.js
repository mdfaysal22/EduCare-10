    ...
    <div className="w-4/5 mx-auto">
      ...
      {features.map((f) => (
        <li className="list-none flex items-center gap-2">
          {" "}
          <FaCheck className="text-xs"></FaCheck> {f}
        </li>
      ))}
      ...
      <div className="my-2">
        ...
        <h3 className="text-xl font-semibold flex items-center gap-2">
          Ratings : {ratings} <FaStar className="text-yellow-600"></FaStar>
      ...
      <Pdf targetRef={ref} filename={name + ".pdf"}>
        {({ toPdf }) =><button onClick={toPdf} className="btn ">
          Download <BsDownload className="ml-2"></BsDownload>
        </button>}
      </Pdf>
        
        <Link to={`/checkout/${id}`} className="btn btn-outline">
          Checkout <FaArrowRight className="ml-2"></FaArrowRight>
        </Link>
      </div>
    </div>
  );
